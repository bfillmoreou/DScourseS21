...
fit            <- rpart(income$high.earner~., data = income, method = 'class')

predict_unseen <- predict(fit, income, type = 'class')

table_mat      <- table(income_test$high.earner, predict_unseen[1:6512])
table_mat
...
bank_matrix = model.matrix(~., data = income)

fmla <- formula(paste("yyes ~ ", paste(colnames(bank_matrix[,-c(1,28)]), collapse= "+"), collapse = ""))

nn1 = neuralnet(tune_nnet_spec, data = bank_matrix,
                hidden = 1,
                algorithm = 'backprop',
                learningrate = 0.0001,
                err.fct="ce",
...
top_acc <- show_best(rec_res, metric = "accuracy")
best_acc <- select_best(rec_res, metric = "accuracy")
final_svm <- finalize_workflow(rec_wf,best_acc)

avm_test <- last_fit(final_svm,income_split) %>%
